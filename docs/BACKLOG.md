...
14. [DOING] Center the board — the board is horizontally centered in the layout. Pure layout, no behavior change.
15. [TODO] Above-board held piece — a piece in the CURRENT player's color sits above the board. It idles with a gentle left–right glide and moves to track the column under the pointer. Presentation only: reads current player and selected colors; no game-logic change. (prefers-reduced-motion: no idle glide.)
