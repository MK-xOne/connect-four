...
2. [TODO] dropToken + tests — `board.ts` `dropToken` and `isFull`, with Vitest covering gravity and full-column cases.
3. [TODO] Click to place + turn switch — clicking a column drops the current player's token; turn alternates.
4. [TODO] checkWin + tests — `win.ts` `checkWin` covering horizontal, vertical, both diagonals, and no-win.
